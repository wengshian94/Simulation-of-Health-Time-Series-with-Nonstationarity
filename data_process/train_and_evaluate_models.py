...
def train_and_eval_metrics_traintestsplit(X, y, model, modelname):
...
def train_and_eval_metrics_LOOCV(X, y, model, modelname):
    loo = LeaveOneOut()
    y_true, y_pred = [], []
...
if __name__ == "__main__":
    ...
    models = {
        ...
        "RF": RandomForestClassifier(n_estimators=50, min_samples_split = 5, class_weight="balanced", random_state=42)
    }
    ...
    for modelname in models.keys():
        ...
        curr_df_metrics = train_and_eval_metrics_LOOCV(X_real_combined, y_real_combined, curr_model, modelname)
    ...
    for modelname in models.keys():
        curr_model = models[modelname]
        curr_df_metrics = train_and_eval_metrics_LOOCV(X_sim, y_sim, curr_model, modelname)
    ...
    for modelname in models.keys():
        curr_model = models[modelname]
        curr_df_metrics = train_and_eval_metrics_LOOCV(X_aug, y_aug, curr_model, modelname)
